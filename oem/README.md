...
> **Важно (Windows 1.2.0-beta.7):** кнопка «Обновление» в beta.7 закрывала приложение вместо установки (zip+`exit` на single-file portable). Сейчас в `update.json` нет `zip`, поэтому beta.7 откроет загрузку `DeskForce.exe` в браузере — скачайте и замените файл один раз. Автоустановка portable EXE исправлена начиная с **1.2.0-beta.14**. Сборки **beta.13/14** снимались с канала из‑за вылета при старте (гонка maximize); стабильный фикс окна — **1.2.0-beta.16**.
